...
export class OcorrenciasComponent implements OnInit {
  ...
  static OcorrenciaEditada(group: AbstractControl): { [key: string]: boolean } {
    ...
    if (
      (!dataOcorrencia || !textoOcorrencia) &&
      (dataOcorrencia.touched ||
        textoOcorrencia.touched ||
        dataOcorrencia.dirty ||
        textoOcorrencia.dirty)
    ) {
      return { ocorrenciaEdicao: false };
    }
    if (
      dataOcorrencia.value !== null &&
      textoOcorrencia !== null &&
      OcorrenciasComponent.edicao
    ) {
    ...
    this.ocorrenciaForm = new FormGroup(
      {
        ...
        textoOcorrencia: new FormControl("", {
    ...
    this.ocorrenciaForm = new FormGroup(
      {
        ...
        textoOcorrencia: new FormControl(textoOcorrencia, {
          validators: [Validators.required, Validators.minLength(5)],
        }),
  ...
  editarRascunho(ocorrencia: any) {
    this.ocorrenciaService
      .editarRascunho(ocorrencia.id)
      .subscribe((ocorrencia) => (ocorrencia = ocorrencia));
    this.setarOcorrencia(
      ocorrencia.id,
  ...
  salvarEdicao(ocorrencia: OcorrenciaDetalhe) {
    ...
    this.ocorrenciaService.salvarEdicao(ocorrencia).subscribe((sucess) => {
      this.carregarDados();
      this.limparOcorrencia();
    });
