...
export class RegistroMarcacaoKmComponent implements OnInit {
  ...
  constructor(
    private kmService: MarcacaoKmService,
    private kmComponent: MarcacaoKmComponent
  ) {}

  ngOnInit() {}

  removerKm(kmRemovido: any) {
    this.kmService.removerKm(kmRemovido).subscribe((despesa) => {
      this.registrosKm.splice(
        this.registrosKm.indexOf(
          this.registrosKm.find((item) => kmRemovido.id === item.id)
        ),
        1
      );
